    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-none flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="bg-[#0f172a] text-white p-8 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-black  tracking-tight">Variações de Cor</h2>
            <p className="text-purple-500 text-[10px] font-bold  mt-1">{productName}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            <div className="p-6 bg-slate-50 border border-slate-100 space-y-5">
              <h3 className="text-[10px] font-black text-slate-500  tracking-widest">Nova Variação de Cor</h3>

              {/* Nome + Cor */}
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-400 ">Nome da Cor</p>
                  <input
                    className="w-full bg-white border border-slate-200 p-3 rounded-none text-xs font-bold  outline-none"
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-400 ">Cor (Hex)</p>
                  <div className="flex items-center gap-2">
                    ...
                    <input className="flex-1 bg-white border border-slate-200 p-3 rounded-none text-xs font-bold outline-none " value={draft.colorHex} onChange={e => setDraft(d => ({ ...d, colorHex: e.target.value }))} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <p className="text-[9px] font-black text-slate-400 ">Foto desta Cor (WebP auto)</p>
                <label className="block border-2 border-dashed border-slate-200 p-5 text-center cursor-pointer hover:border-purple-400 transition-all">
                  <div className="flex items-center justify-center gap-4">
                    {draft.imageUrl ? (
                      ...
                      <div className="w-16 h-16 bg-slate-100 flex items-center justify-center rounded-none">
                        {uploading === "draft" ? <Loader2 size={20} className="animate-spin text-purple-500" /> : <Camera size={24} className="text-slate-300" />}
                      </div>
                    )}
              ...
              <div className="space-y-2">
                <div className="flex gap-2 flex-wrap">
                  {(Object.keys(SIZE_PRESETS) as Array<keyof typeof SIZE_PRESETS>).map(p => (
                    <button key={p} onClick={() => setSizePreset(p)} className={`px-3 py-1 text-[9px] font-black  rounded-none border transition-all ${sizePreset === p ? 'bg-purple-500 text-white border-purple-500' : 'bg-white text-slate-400 border-slate-200'}`}>
                      {p === "clothes" ? "Roupas" : p === "shoes" ? "Calçados" : "Acessórios"}
                    </button>
                  ))}
                </div>
                <p className="text-[9px] font-black text-slate-400 ">Tamanhos Disponíveis</p>
                <div className="flex flex-wrap gap-2">
                  {SIZE_PRESETS[sizePreset].map(size => (
                    <button key={size} onClick={() => toggleSize(size)} className={`w-12 h-12 text-[10px] font-black  rounded-none border-2 transition-all ${draft.sizes.includes(size) ? 'bg-[#0f172a] text-white border-[#0f172a]' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-400'}`}>
                      {size}
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-400 ">Estoque (Qtd)</p>
                ...
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-slate-400 ">Código de Barras</p>
                  <input type="text" className="w-full bg-white border border-slate-200 p-3 rounded-none text-xs font-bold outline-none " placeholder="OPCIONAL" value={draft.barcode || ""} onChange={e => setDraft(d => ({ ...d, barcode: e.target.value }))} />
                </div>
              </div>

              <button onClick={saveVariant} className="w-full bg-purple-500 text-white py-4 rounded-none font-black text-xs  tracking-widest border-none shadow-lg hover:brightness-110 transition-all">
          ...
          <div className="space-y-4">
            <h3 className="text-[10px] font-black text-slate-500  tracking-widest">({variants.length}) Variações Cadastradas</h3>
            {loading ? (
              <div className="flex items-center justify-center py-10"><Loader2 className="animate-spin text-purple-500" size={24} /></div>
            ) : variants.length === 0 ? (
              <div className="text-center py-10 text-slate-300">
                <Palette size={40} className="mx-auto mb-2" />
                <p className="text-xs font-bold ">Nenhuma variante cadastrada</p>
              </div>
            ) : (
              variants.map(v => (
                <div key={v.id} className="bg-white border border-slate-100 p-5 flex items-center gap-4 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-black text-slate-900  text-sm">{v.color}</p>
                    <div className="flex flex-wrap gap-1 mt-2">
                      {v.sizes.map(s => (
                        <span key={s} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[9px] font-black  rounded-none">{s}</span>
                      ))}
                    </div>
