...
export default function ShowcaseCatalog({ store, products, categories }: ShowcasePageProps) {
  ...
  function getStockInfo(product: Product) {
    if (product.variants && product.variants.length > 0) {
      ...
      return { total: totalStock, hasVariants: true };
    }
    return { total: product.stock || 0, hasVariants: false };
  }

  function sendToWhatsApp() {
    if (!selectedProduct) return;
    
    const stockInfo = getStockInfo(selectedProduct);
    const currentStock = selectedVariant ? (selectedVariant.stock || 0) : stockInfo.total;

    if (currentStock <= 0) {
      toast.error("Desculpe, este item acabou de esgotar.");
      return;
    }
    ...
    const message = `✨ *NOVO INTERESSE EM PRODUTO* ✨\n\n` +
      `Olá! Estive visitando sua vitrine e amei este item:\n\n` +
      `🛍️ *Produto:* ${selectedProduct.name}\n` +
      (selectedVariant ? `🎨 *Variação:* ${selectedVariant.color}\n` : "") +
      (selectedSize ? `📏 *Tamanho:* ${selectedSize}\n` : "") +
      `💰 *Preço:* R$ ${(selectedProduct.salePrice || selectedProduct.price).toFixed(2).replace('.', ',')}\n\n` +
      `🔗 *Link:* ${storeUrl}\n\n` +
      `Poderia me informar sobre a disponibilidade e formas de entrega? 😊`;
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
              <div className="w-20 h-20 md:w-32 md:h-32 bg-white border-4 border-white -mt-16 md:-mt-20 shadow-xl overflow-hidden flex-shrink-0 rounded-[2rem] relative z-20">
                {store.logo ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-slate-50 text-slate-200"><ShoppingBag size={48} /></div>
                )}
              </div>
              <div className="flex-1 text-center md:text-left">
                <h1 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter">{store.name}</h1>
                <p className="text-slate-500 text-sm mt-2 leading-relaxed max-w-2xl">{store.description || "Tendências e novidades exclusivas para você."}</p>
                <div className="flex flex-wrap justify-center md:justify-start gap-6 mt-6">
                   <button onClick={() => setShowHours(true)} className="flex items-center gap-2 hover:opacity-80 transition-all group">
                      <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-slate-400 group-hover:text-brand transition-colors"><Clock size={18} /></div>
                      <div className="text-left">
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Horários</p>
                   ...
                   <a href={`https://wa.me/${store.whatsapp}`} target="_blank" className="flex items-center gap-2 hover:opacity-80 transition-all group">
                      ...
                      <div className="text-left">
                         ...
                         <p className="text-xs font-bold text-slate-700">Falar Agora</p>
                      </div>
                   </a>
                </div>
              </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
            ...
            return (
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden">
                  ...
                  {(() => {
                    const { total } = getStockInfo(product);
                    if (total <= 0) {
                      return (
                        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-[2px] flex items-center justify-center z-10">
                          <span className="bg-white text-slate-900 text-[10px] font-black px-4 py-2 rounded-xl uppercase tracking-widest shadow-xl">Esgotado</span>
                        </div>
                      );
                    }
                    if (total > 0 && total < 5) {
                      return (
                        <div className="absolute top-4 left-4 bg-amber-500 text-white text-[8px] font-black px-2 py-1 rounded-lg shadow-lg uppercase tracking-tighter animate-pulse">
                          Últimas Unidades
                        </div>
                      );
                    }
        ...
        {filteredProducts.length === 0 && (
          <div className="py-32 text-center flex flex-col items-center gap-4 opacity-20">
            ...
            <p className="text-sm font-black uppercase tracking-[0.3em]">Nenhum item encontrado</p>
          </div>
        )}
      </div>
      ...
      <footer className="py-20 text-center opacity-20">
        <div className="flex items-center justify-center gap-2 mb-2">
           ...
           <p className="text-[10px] font-black uppercase tracking-[0.5em]">PedeUe Vitrine</p>
           <div className="h-[1px] w-12 bg-slate-400"></div>
        </div>
      </footer>
              ...
              <div className="flex-1 p-8 lg:p-16 flex flex-col bg-white">
                <div className="mb-10">
                   <div className="flex items-center gap-3 mb-4">
                      <span className="bg-slate-100 text-[10px] font-black text-slate-500 px-3 py-1 rounded-lg uppercase tracking-widest">
                         {categories.find(c => c.id === selectedProduct.categoryId)?.name || "Tendência"}
                      </span>
                      {selectedProduct.salePrice && <span className="bg-brand/10 text-brand text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-widest">Em Oferta</span>}
                   </div>
                   ...
                   <div className="mt-4 flex items-center gap-2">
                      {(() => {
                        const stock = selectedVariant ? (selectedVariant.stock || 0) : (selectedProduct.stock || 0);
                        if (stock <= 0) return <span className="text-xs font-black text-red-500 uppercase tracking-widest flex items-center gap-1"><X size={14} /> Fora de estoque</span>;
                        if (stock < 5) return <span className="text-xs font-black text-amber-500 uppercase tracking-widest flex items-center gap-1"><Clock size={14} /> Apenas {stock} unidades disponíveis!</span>;
                        return <span className="text-xs font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1"><CheckCircle2 size={14} /> Em estoque ({stock} unidades)</span>;
                      })()}
                   </div>
                </div>

                <div className="bg-slate-50 rounded-3xl p-8 mb-10 border border-slate-100">
                   <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2"><Info size={14} /> Descrição do Produto</h4>
                   <p className="text-sm text-slate-600 leading-relaxed font-medium italic">
                      {selectedProduct.description || "Nenhum detalhe adicional fornecido para este item exclusivo."}
                   </p>
                </div>
                ...
                <div className="space-y-10 mb-12">
                   {selectedProduct.variants && selectedProduct.variants.length > 0 && (
                     <div>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Selecione uma Variação</p>
                   ...
                   {selectedVariant && selectedVariant.sizes.length > 0 && (
                     <div>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Escolha o Tamanho: <span className="text-brand">{selectedSize || "—"}</span></p>
                ...
                <button
                  ...
                  disabled={(() => {
                    const stock = selectedVariant ? (selectedVariant.stock || 0) : (selectedProduct.stock || 0);
                    return stock <= 0 || (!selectedSize && (selectedVariant?.sizes?.length || 0) > 0);
                  })()}
                ...
                >
                  ...
                  {(() => {
                    const stock = selectedVariant ? (selectedVariant.stock || 0) : (selectedProduct.stock || 0);
                    return stock <= 0 ? "PRODUTO ESGOTADO" : "TENHO INTERESSE NESTE ITEM";
                  })()}
                </button>
              </div>
            </div>
