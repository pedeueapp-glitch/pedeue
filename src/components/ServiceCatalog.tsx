...
export default function ServiceCatalog({ store, products, categories }: ServicePageProps) {
  ...
  function sendToWhatsApp() {
    ...
    const msg = encodeURIComponent(
      `Olá! Tenho interesse neste serviço:\n\n` +
      `*${selectedProduct.name}*\n` +
      (selectedVariant ? `Opção: ${selectedVariant.color}\n` : "") +
      (selectedSize ? `Detalhe: ${selectedSize}\n` : "") +
      `Valor: R$ ${(selectedProduct.salePrice || selectedProduct.price).toFixed(2)}\n\n` +
      `Pode me passar mais informações sobre a disponibilidade?`
    );
    ...
    <div className="min-h-screen" style={{ backgroundColor: `${primaryColor}05` }}>
         ...
         <div className="flex flex-col items-center -mt-12 md:-mt-16">
            ...
            <div className="mt-4 text-center">
               ...
               <p className="text-slate-400 text-[11px] font-light tracking-[0.15em] max-w-sm mx-auto">
                  {store.description || "Soluções profissionais para você"}
               </p>
            </div>
         </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
               ...
               return (
                 ...
                 >
                    <div className="relative aspect-[4/5] bg-white overflow-hidden rounded-md mb-4 shadow-sm group-hover:shadow-md transition-all border border-slate-100/50">
                       ...
                       <div className="absolute bottom-3 left-3 bg-white/90 backdrop-blur-sm text-[8px] font-bold px-2 py-1 rounded-sm tracking-tighter" style={{ color: primaryColor }}>
                         Serviço
                       </div>
                    </div>
         ...
         {filteredProducts.length === 0 && (
            <div className="text-center py-20 bg-white/50 rounded-lg border border-dashed border-slate-200">
               <Package size={48} className="mx-auto text-slate-100 mb-4" />
               <p className="text-[10px] font-medium tracking-widest text-slate-400">Nenhum serviço encontrado</p>
            </div>
         )}
      </div>
          ...
          <div className="relative flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 p-6 md:p-10 flex flex-col bg-white">
               ...
               <div className="mb-10 text-slate-500 text-[11px] leading-relaxed font-light">
                  {selectedProduct.description || "Solicite um orçamento para mais detalhes."}
               </div>

               {/* Variantes / Opções se houver */}
               {selectedProduct.variants && selectedProduct.variants.length > 0 && (
                 <div className="space-y-8 mb-10">
                    <div>
                      <p className="text-[10px] font-medium text-slate-400 tracking-widest mb-4">Opções Disponíveis</p>
                    ...
                    {selectedVariant && selectedVariant.sizes.length > 0 && (
                      <div>
                        <p className="text-[10px] font-medium text-slate-400 tracking-widest mb-4">Detalhe: <span className="text-slate-800">{selectedSize || "Selecione"}</span></p>
