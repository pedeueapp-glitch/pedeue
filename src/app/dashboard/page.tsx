...
export default function DashboardPage() {
  ...
  const handleRenew = async () => {
    if (!store?.subscription?.planId) return;
    setIsRenewing(true);
    try {
      ...
      <div className="p-4 lg:p-6 max-w-7xl mx-auto w-full space-y-6">
        ...
        {store?.subscription && (() => {
          ...
          if (isExpired) {
            ...
            bgColor = "bg-orange-500/10 border-orange-500/20 text-orange-600";
            ...
            <div className={`p-4 rounded-xl border flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm ${bgColor}`}>
               ...
               {(isExpiringSoon || isExpired) && (
                 <button 
                  onClick={handleRenew}
                  disabled={isRenewing}
                  className="w-full md:w-auto px-6 py-2.5 bg-orange-500 text-white text-[10px] font-black uppercase rounded-lg hover:bg-orange-600 transition-all flex items-center justify-center gap-2 shadow-lg shadow-orange-500/20"
                 >
                   {isRenewing ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
                   Renovar Agora com Mercado Pago
                 </button>
               )}
            </div>
          );
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
           <div className="flex items-center gap-2 text-slate-600">
              ...
              <span className="text-sm font-black uppercase tracking-widest text-navy">Performance da Loja</span>
           </div>
           ...
           <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <button 
                ...
                className="bg-orange-500 text-white p-2.5 rounded-lg hover:bg-orange-600 transition-all shadow-md shadow-orange-500/20"
              >
                 <Search size={16} />
              </button>
           </div>
        ...
        {loading ? (
          <div className="py-24 flex flex-col items-center justify-center">
            ...
            <span className="text-xs font-black uppercase tracking-tighter text-slate-400">Sincronizando faturamento...</span>
          </div>
        ) : (
          <>
            {/* INDICADORES (KPIs) */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
               {[
                { label: "Faturamento", value: formatCurrency(data?.summary?.totalRevenue || 0), icon: DollarSign, color: "text-orange-500", bg: "bg-orange-50" },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
               <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                  <div className="flex items-center justify-between mb-8">
                     <h3 className="text-sm font-black text-navy uppercase tracking-widest">Evolução do Faturamento</h3>
                     <div className="flex items-center gap-2">
                        <div className="w-2 h-2 bg-orange-500 rounded-full"></div>
                        <span className="text-[10px] font-bold text-slate-400 uppercase">Vendas Diárias</span>
                     </div>
                  </div>
               ...
               <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                  <h3 className="text-sm font-black text-navy uppercase tracking-widest mb-8">Meios de Pagamento</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                   <div className="p-4 border-b border-slate-100 flex items-center bg-slate-50/30">
                      ...
                      <h3 className="text-xs font-black text-navy uppercase tracking-widest">Maiores Compradores</h3>
                   </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                   <div className="p-4 border-b border-slate-100 flex items-center bg-slate-50/30">
                      ...
                      <h3 className="text-xs font-black text-navy uppercase tracking-widest">Itens mais Vendidos</h3>
                   </div>
                      ...
                      {data?.topProducts?.slice(0, 5).map((p: any, i: number) => (
                        <div key={i} className="flex items-center justify-between">
                           ...
                           <div className="flex items-center gap-4">
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{p.quantity} vendas</span>
                   ...
                   <div className="p-4 bg-orange-50 border-t border-orange-100">
                      <div className="flex items-center justify-between text-[10px] font-black uppercase text-orange-600">
