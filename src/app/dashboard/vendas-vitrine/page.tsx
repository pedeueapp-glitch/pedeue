      ...
      <div className="p-6 lg:p-10 max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex items-center gap-3">
            <button 
              ...
              className={`px-5 py-3 font-black uppercase text-[10px] tracking-widest border-2 transition-all rounded-none flex items-center gap-2 ${filter === "today" ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-200 text-slate-500 hover:border-slate-400'}`}
            >
              <Calendar size={14} /> Hoje
            </button>
            <button 
              onClick={() => setFilter("all")}
              className={`px-5 py-3 font-black uppercase text-[10px] tracking-widest border-2 transition-all rounded-none flex items-center gap-2 ${filter === "all" ? 'border-orange-500 bg-orange-500 text-white' : 'border-slate-200 text-slate-500 hover:border-slate-400'}`}
            >
          ...
          <div className="relative w-full md:w-80">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3 border-2 border-slate-200 font-bold text-xs uppercase outline-none focus:border-orange-500 rounded-none transition-all"
              value={searchTerm}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="p-6 bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Faturamento Total</p>
            </div>
          ...
          <div className="p-6 bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              ...
              <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Total de Vendas</p>
            </div>
            <p className="text-2xl font-black text-blue-700">{totals.totalSales}</p>
          </div>
          <div className="p-6 bg-white border border-slate-100 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 bg-orange-50 border border-orange-100 flex items-center justify-center">
                <Package size={18} className="text-orange-600" />
              </div>
              <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Itens Vendidos</p>
            </div>
            <p className="text-2xl font-black text-orange-700">{totals.totalItems}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-100 shadow-sm">
          ...
          <div className="p-5 border-b border-slate-100 flex items-center gap-3">
            ...
            <h3 className="font-black uppercase text-sm text-slate-800 tracking-tight">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-xs font-black uppercase text-slate-400 tracking-widest">Carregando...</p>
            </div>
          ) : filteredSales.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-300">
              <Receipt size={48} className="mb-4 opacity-50" />
              <p className="font-black uppercase text-xs tracking-widest">Nenhuma venda encontrada</p>
            </div>
          ) : (
                ...
                <div key={sale.id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="font-black text-slate-800 text-xs uppercase">{sale.customerName || "Venda Balcao"}</h4>
                        <span className="bg-green-100 text-green-700 text-[8px] font-black px-2 py-0.5 uppercase tracking-widest">{sale.status}</span>
                        <span className="bg-slate-100 text-slate-500 text-[8px] font-black px-2 py-0.5 uppercase tracking-widest">{sale.paymentMethod}</span>
                      </div>
                  ...
                  {selectedSale?.id === sale.id && (
                    <div className="px-5 pb-5 bg-slate-50/50 border-t border-slate-100">
                      <p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mt-4 mb-3">
                        ...
                        {sale.items?.map((item: any) => (
                          <div key={item.id} className="bg-white p-4 border border-slate-100 flex items-center gap-4">
                            ...
                            <div className="text-right shrink-0">
                              ...
                              <p className="font-black text-orange-600 text-sm">R$ {(item.quantity * item.price).toFixed(2).replace('.', ',')}</p>
                            </div>
                          </div>
                        ))}
                      </div>
